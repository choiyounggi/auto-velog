---
name: setup
description: auto-velog 최초 설정. 데이터 디렉토리 생성, config 작성, 플랫폼 로그인(쿠키 저장), 비공개 테스트 발행까지 안내. "auto-velog 설정", "auto-velog setup", "블로그 자동화 설정" 등에 트리거.
---
...
```bash
mkdir -p ~/.auto-velog/{queue,drafts,secrets}
[ -f ~/.auto-velog/config.json ] || cp "$PLUGIN/config.example.json" ~/.auto-velog/config.json
```

사용자에게 물어 `~/.auto-velog/config.json`을 채운다:
...
`~/.auto-velog/secrets/`로 복사해 재사용한다 (재로그인 불필요). 없으면:
...
```bash
cat > ~/.auto-velog/drafts/setup-test.md <<'MD'
---
title: auto-velog 설정 테스트 (비공개)
tags: test
---
auto-velog 파이프라인 테스트 글입니다. 보이면 성공.
MD
node "$PLUGIN/scripts/secret-scan.mjs" ~/.auto-velog/drafts/setup-test.md \
  && node "$PLUGIN/scripts/adapters/velog/publish.mjs" ~/.auto-velog/drafts/setup-test.md --private
```
...
- 이제 아무 세션에서나 글감이 완결되면 자동으로 포착 → 세션 종료 시 초안
  파이프라인이 돈다 (`~/.auto-velog/autodraft.log`)
- 일시 정지: `touch ~/.auto-velog/PAUSE` / 재개: `rm ~/.auto-velog/PAUSE`
- 완전 비활성화: 환경변수 `AUTO_VELOG_AUTODRAFT=0`
- 밀린 글감 수동 처리: `/auto-velog:flush`
