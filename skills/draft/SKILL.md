...
**보충 리서치 (허용):** 세션 내용만으로 본문이 빈약하거나 배경 설명이 필요하면
WebSearch/WebFetch로 조사한 내용을 정리해 추가해도 된다. 단:
- 보충은 심사를 통과한 글감을 살찌우는 용도다 — 완결성 0점 글감을 리서치로
  살려내지 말 것 (심사 기준은 그대로)
- 세션에서 실제 겪은 일과 외부 자료를 뒤섞어 "내가 겪은 것처럼" 쓰지 말 것.
  경험은 경험대로, 배경지식은 배경지식대로 자연스럽게 구분되게
- 외부 자료를 근거로 쓴 부분은 출처 링크를 남긴다
