#!/usr/bin/env bash
# auto-velog — SessionStart: ★ BlogWorthy 블록 발현 지침 주입.
#
# 블록 구분선(U+2500)과 필드명은 scripts/harvest.mjs 의 BLOCK_RE/FIELD_KEYS 와
# 정확히 동기화되어야 한다.
set +e

# 재귀 방지: headless draft 세션이거나 auto-velog 데이터 디렉토리 안이면 skip
[ -n "${AUTO_VELOG_DRAFTING:-}" ] && exit 0
case "${CLAUDE_PROJECT_DIR:-$PWD}" in
  "$HOME/.auto-velog"*) exit 0 ;;
esac

cat <<'EOF'
# auto-velog — 블로그 글감 포착 (standing instruction)
