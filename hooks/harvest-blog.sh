#!/usr/bin/env bash
# blog-loop — Stop hook: ★ BlogWorthy 블록을 큐로 수확하고, 새 글감이 있으면
# detached headless claude 로 draft 파이프라인을 spawn 한다.
#
# 가드 (auto-flush.sh 패턴):
#   - 킬 스위치:  BLOG_LOOP_AUTODRAFT=0 또는 ~/.blog-loop/PAUSE 파일
#   - 재귀 방지:  BLOG_LOOP_DRAFTING=1 (headless draft 세션), stop_hook_active
#   - rate limit: BLOG_LOOP_AUTODRAFT_INTERVAL 초당 1회 (기본 1800)
...
# --- draft spawn 가드 ------------------------------------------------------
[ "${BLOG_LOOP_AUTODRAFT:-1}" = "0" ] && exit 0
[ -f "$DIR/PAUSE" ] && exit 0
command -v claude >/dev/null 2>&1 || exit 0

# rate limit
STAMP="$DIR/.last-autodraft"
INTERVAL_SEC="${BLOG_LOOP_AUTODRAFT_INTERVAL:-1800}"
